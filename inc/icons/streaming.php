...
function bilalmghl_streaming_sets(){
	$icons = array(
...
class bilalmghl_Add_Streaming_Icons {
    
    public function __construct() { 
		add_action( 'elementor/editor/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_streaming' ] );
		add_action( 'elementor/frontend/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_streaming' ] );    
		add_filter( 'elementor/icons_manager/additional_tabs', [ $this, 'bilalmghl_elementor_streaming_setup' ] );
	}
    
    public function bilalmghl_enqueue_streaming(){
        wp_enqueue_style( 'streaming', BILALMGHL_ADDONS_ROOT_ICON . 'streaming/streaming.css', array(), '1.0.1' );
    }

	public function bilalmghl_elementor_streaming_setup( $tabs = array()){

		$new_icons = bilalmghl_streaming_sets();

		$tabs['streaming'] = array(
			'name'          => 'streaming',
			'label'         => esc_html__( 'Online Streaming', 'bilalmghl' ),
			'labelIcon'     => 'streaming-039-spotlight',
			'prefix'        => '',
			'displayPrefix' => 'streaming',
			'url'           => BILALMGHL_ADDONS_ROOT_ICON . 'streaming/streaming.css',
			'icons'         => $new_icons,
			'ver'           => '1.0.0',
		);
		return $tabs;
...
new bilalmghl_Add_Streaming_Icons();