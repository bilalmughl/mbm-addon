...
function bilalmghl_education_sets(){
	$icons = array(
...
class bilalmghl_Add_Education_Icons {
    
    public function __construct() { 
		add_action( 'elementor/editor/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_education' ] );
		add_action( 'elementor/frontend/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_education' ] );    
		add_filter( 'elementor/icons_manager/additional_tabs', [ $this, 'bilalmghl_elementor_education_setup' ] );
	}
    
    public function bilalmghl_enqueue_education(){
        wp_enqueue_style( 'education', BILALMGHL_ADDONS_ROOT_ICON . 'education/education.css', array(), '1.0.1' );
    }

	public function bilalmghl_elementor_education_setup( $tabs = array()){

		$new_icons = bilalmghl_education_sets();

		$tabs['education'] = array(
			'name'          => 'education',
			'label'         => esc_html__( 'Education Icons', 'bilalmghl' ),
			'labelIcon'     => 'education-001-work-space',
			'prefix'        => '',
			'displayPrefix' => 'education',
			'url'           => BILALMGHL_ADDONS_ROOT_ICON . 'education/education.css',
			'icons'         => $new_icons,
			'ver'           => '1.0.0',
		);
		return $tabs;
...
new bilalmghl_Add_Education_Icons();