...
function bilalmghl_hospital_sets(){
	$icons = array(
...
class bilalmghl_Add_Hospital_Icons {
    
    public function __construct() { 
		add_action( 'elementor/editor/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_hospital' ] );
		add_action( 'elementor/frontend/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_hospital' ] );    
		add_filter( 'elementor/icons_manager/additional_tabs', [ $this, 'bilalmghl_elementor_hospital_setup' ] );
	}
    
    public function bilalmghl_enqueue_hospital(){
        wp_enqueue_style( 'hospital', BILALMGHL_ADDONS_ROOT_ICON . 'hospital/hospital.css', array(), '1.0.1' );
    }

	public function bilalmghl_elementor_hospital_setup( $tabs = array()){

		$new_icons = bilalmghl_hospital_sets();

		$tabs['hospital'] = array(
			'name'          => 'hospital',
			'label'         => esc_html__( 'Hospital Icons', 'bilalmghl' ),
			'labelIcon'     => 'hospital-009-hospital',
			'prefix'        => '',
			'displayPrefix' => 'hospital',
			'url'           => BILALMGHL_ADDONS_ROOT_ICON . 'hospital/hospital.css',
			'icons'         => $new_icons,
			'ver'           => '1.0.0',
		);
		return $tabs;
...
new bilalmghl_Add_Hospital_Icons();