...
function bilalmghl_audiovideo_sets(){
	$icons = array(
...
class bilalmghl_Add_Audiovideo_Icons {
    
    public function __construct() { 
		add_action( 'elementor/editor/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_audiovideo' ] );
		add_action( 'elementor/frontend/after_enqueue_styles', [ $this, 'bilalmghl_enqueue_audiovideo' ] );    
		add_filter( 'elementor/icons_manager/additional_tabs', [ $this, 'bilalmghl_elementor_audiovideo_setup' ] );
	}
    
    public function bilalmghl_enqueue_audiovideo(){
        wp_enqueue_style( 'audiovideo', BILALMGHL_ADDONS_ROOT_ICON . 'audiovideo/audiovideo.css', array(), '1.0.1' );
    }

	public function bilalmghl_elementor_audiovideo_setup( $tabs = array()){

		$new_icons = bilalmghl_audiovideo_sets();

		$tabs['audiovideo'] = array(
			'name'          => 'audiovideo',
			'label'         => esc_html__( 'Audiovideo Icons', 'bilalmghl' ),
			'labelIcon'     => 'audiovideo-025-rgb',
			'prefix'        => '',
			'displayPrefix' => 'audiovideo',
			'url'           => BILALMGHL_ADDONS_ROOT_ICON . 'audiovideo/audiovideo.css',
			'icons'         => $new_icons,
			'ver'           => '1.0.0',
		);
		return $tabs;
...
new bilalmghl_Add_Audiovideo_Icons();