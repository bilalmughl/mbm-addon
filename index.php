...
/**
 * Plugin Name: MBM Addon For Elementor
 * Description: MBM Addon For Elementor is a full plugin for colleciton of elementor widgets.
 * Plugin URI: https://plugins.bilalmghl.com/mbm-addon
 * Author: Muhammad Bilal Mughal
 * Author URI: https://bilalmghl.com
 * Version: 1.0.0
 * License: GPL2
 * Text Domain: bilalmghl
 ...
 */
define( 'BILALMGHL_VERSION', '1.1.0' );
define( 'BILALMGHL_ADDONS_ROOT', dirname(__FILE__) );

define( 'BILALMGHL_ADDONS_URL', plugins_url( '/', __FILE__ ) );
define( 'BILALMGHL_ADDONS_ROOT_JS', plugins_url( '/assets/js/', __FILE__ ) );
define( 'BILALMGHL_ADDONS_ROOT_CSS', plugins_url( '/assets/css/', __FILE__ ) );
define( 'BILALMGHL_ADDONS_ROOT_ICON', plugins_url( '/assets/icons/', __FILE__ ) );
define( 'BILALMGHL_ADDONS_ROOT_IMG', plugins_url( '/assets/img/', __FILE__ ) );

define( 'BILALMGHL_ADDONS_DIR_URL', plugin_dir_url( __FILE__ ));
define( 'BILALMGHL_ADDONS_DIR_PATH', plugin_dir_path( __FILE__ ) );
define( 'BILALMGHL_ADDONS_BASE', plugin_basename( BILALMGHL_ADDONS_ROOT ) );
/*OSCAR THEMES PATH*/
define( 'OSCAR_ADDONS_ROOT_CSS', plugins_url( '/oscar/assets/css/', __FILE__ ) );
# define( 'OSCAR_ADDONS_ROOT_JS', plugins_url( '/oscar/assets/js/', __FILE__ ) );
define( 'OSCAR_ADDONS_ROOT_JS', plugins_url( '/oscar/assets/js/', __FILE__ ) );
final class Bilalmghl_Elementor_Extension {
	...
	public function init() {
		...
		/*oscar*/
		if (file_exists(dirname(__FILE__) . '/oscar/oscar.php' )) {
			require_once(dirname(__FILE__) . '/oscar/oscar.php' );
		}
	}
	...
	public function bilalmghl_editor_styles(){
		wp_enqueue_style( 'bilalmghl-editor', BILALMGHL_ADDONS_ROOT_CSS . 'bilalmghl-editor.css' );
	}

	public function bilalmghl_default_scripts(){
		wp_enqueue_style( 'bilalmghl-widgets', BILALMGHL_ADDONS_ROOT_CSS . 'widgets.css' );
		/*OSCAR WIDGETS CSS*/
		wp_enqueue_style( 'oscar-widgets', OSCAR_ADDONS_ROOT_CSS . 'widgets.css' );
		if ( class_exists('Give') ) {
			wp_enqueue_style( 'overwrite', BILALMGHL_ADDONS_ROOT_CSS . 'overwrite.css', array('give-styles'), BILALMGHL_VERSION, 'all' );
		}
	}
	...
	public function bilalmghl_enqueue_frontend_scripts(){
        wp_enqueue_script( 'appear', BILALMGHL_ADDONS_ROOT_JS . 'appear.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_enqueue_script( 'waypoints', BILALMGHL_ADDONS_ROOT_JS . 'waypoints.min.js', array('jquery'), BILALMGHL_VERSION, true );
	}
	...
	public function bilalmghl_register_frontend_scripts() {
        ...
        wp_register_script( 'owl-carousel', BILALMGHL_ADDONS_ROOT_JS . 'owl.carousel.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'slick', BILALMGHL_ADDONS_ROOT_JS . 'slick.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'swiper', BILALMGHL_ADDONS_ROOT_JS . 'swiper.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'modal-video', BILALMGHL_ADDONS_ROOT_JS . 'modal-video.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'svg-progress', BILALMGHL_ADDONS_ROOT_JS . 'svg-progress-min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'TimeCircle', BILALMGHL_ADDONS_ROOT_JS . 'TimeCircles.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'roadmap', BILALMGHL_ADDONS_ROOT_JS . 'roadmap.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'timeline', BILALMGHL_ADDONS_ROOT_JS . 'timeline.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'tooltipster', BILALMGHL_ADDONS_ROOT_JS . 'tooltipster.bundle.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'animatedheadline', BILALMGHL_ADDONS_ROOT_JS . 'jquery.animatedheadline.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'easyBar', BILALMGHL_ADDONS_ROOT_JS . 'easyBar.js', array('jquery','waypoints'), BILALMGHL_VERSION, true );

        /*--------------------------
			SINGLE SCRIPTS
        ---------------------------*/
        wp_register_script( 'isotope', BILALMGHL_ADDONS_ROOT_JS . 'isotope.pkgd.min.js', array('jquery','imagesloaded'), BILALMGHL_VERSION, true );
        wp_register_script( 'masonry', array('jquery', 'imagesloaded') );
        wp_register_script( 'ajaxchimp', BILALMGHL_ADDONS_ROOT_JS . 'ajaxchimp.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'anime', BILALMGHL_ADDONS_ROOT_JS . 'anime.min.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'bilalmghl-effect', BILALMGHL_ADDONS_ROOT_JS . 'bilalmghl-effect.min.js', array('jquery'), BILALMGHL_VERSION, true );
        ...
        wp_register_script( 'base_effect', BILALMGHL_ADDONS_ROOT_JS . 'bilalmghl_base_effect.js', array('jquery'), BILALMGHL_VERSION, true );
        //wp_register_script( 'base_effect_2', BILALMGHL_ADDONS_ROOT_JS . 'bilalmghl_base_effect_2.js', array('jquery'), BILALMGHL_VERSION, true );
        

        wp_register_script( 'bilalmghl-core', BILALMGHL_ADDONS_ROOT_JS . 'active.js', array('jquery'), BILALMGHL_VERSION, true );
        wp_register_script( 'oscar-core', OSCAR_ADDONS_ROOT_JS . 'active.js', array('jquery'), BILALMGHL_VERSION, true );
	...
	public function bilalmghl_enqueue_frontend_style() {
        ...
        wp_enqueue_style( 'be-animate', BILALMGHL_ADDONS_ROOT_CSS . 'animate.css', array('e-animations') );
	}
	...
	public function bilalmghl_register_frontend_styles(){
		wp_register_style( 'owl-carousel', BILALMGHL_ADDONS_ROOT_CSS .'owl.carousel.css' );
        wp_register_style( 'slick', BILALMGHL_ADDONS_ROOT_CSS .'slick.min.css' );
        wp_register_style( 'swiper', BILALMGHL_ADDONS_ROOT_CSS .'swiper.min.css' );
        wp_register_style( 'modal-video', BILALMGHL_ADDONS_ROOT_CSS .'modal-video.min.css' );
        wp_register_style( 'TimeCircle', BILALMGHL_ADDONS_ROOT_CSS .'TimeCircles.css' );
        wp_register_style( 'roadmap', BILALMGHL_ADDONS_ROOT_CSS .'roadmap.min.css' );
        wp_register_style( 'timeline', BILALMGHL_ADDONS_ROOT_CSS .'timeline.min.css' );
        wp_register_style( 'tooltipster', BILALMGHL_ADDONS_ROOT_CSS .'tooltipster.bundle.min.css' );
        wp_register_style( 'animatedheadline', BILALMGHL_ADDONS_ROOT_CSS .'jquery.animatedheadline.css' );
        wp_register_style( 'easyBar', BILALMGHL_ADDONS_ROOT_CSS .'easyBar.css' );
	}
	...
	public function add_elementor_category()
	{
		...
		Plugin::instance()->elements_manager->add_category( 'oscar-addons', array(
			'title' => __( 'Oscar Addons', 'bilalmghl' ),
			'icon'  => 'fa fa-plug',
		), 2 );
	}
	...
	******************************/
	public function includes() {
		...
		public function oscar_widgets(){
			/*---------------------------
				Include Widget files
			-----------------------------*/
			
			//***************************OSCAR********************
			if ( array_key_exists( 'welcome_slides', oscar_widget_control() ) ){
			    require_once( __DIR__ . '/oscar/widgets/welcome_slides.php' );
			}
			if ( array_key_exists( 'timeline_slides', oscar_widget_control() ) ){
			    require_once( __DIR__ . '/oscar/widgets/timeline_slides.php' );
			}
			if ( array_key_exists( 'oscar_post_carousel', oscar_widget_control() ) ){
			    require_once( __DIR__ . '/oscar/widgets/oscar_post_carousel.php' );
			}
			if ( array_key_exists( 'area_title', oscar_widget_control() ) ){
			    require_once( __DIR__ . '/oscar/widgets/area_title.php' );
			}
			 if (array_key_exists( 'subscriber_form', bilalmghl_widget_control()) ){
			    require_once( __DIR__ . '/oscar/widgets/subscriber_form.php' );
			}
